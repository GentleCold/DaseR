...
from daser.connector.store_pipeline import StagedStoreBatch, StorePipeline
...
class WorkerRuntime:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]) -> None:
        ...
        if kv_caches:
            (
                self._store_pipeline.staging_bytes,
                self._store_pipeline.pending_staging_limit_bytes,
            ) = _derive_staging_limits(sample.device)
            logger.info(
        ...
        if sample is not None:
            self._store_pipeline.staging_bytes = max(
                self._store_pipeline.staging_bytes or DEFAULT_STORE_STAGING_BYTES,
                self._local_slot_size,
            )
            self._store_pipeline.staging_pool = FixedCudaStagingPool(
                device=sample.device,
                buffer_bytes=self._store_pipeline.staging_bytes,
                depth=_store_staging_pool_depth(
                    self._store_pipeline.staging_bytes,
                    self._store_pipeline.pending_staging_limit_bytes,
                ),
            )
            self._load_pipeline.set_staging_pool(
                FixedCudaStagingPool(
                    device=sample.device,
                    buffer_bytes=self._store_pipeline.staging_bytes,
            ...
            self._load_pipeline.staging_registered = False
            logger.info(
                ...
                self._store_pipeline.pending_staging_limit_bytes,
                _LOAD_REQUEST_MAX_INFLIGHT,
                self._load_pipeline.staging_pool.depth,
            )
            if sample.dim() >= 5:
    ...
    def register_cross_layers_kv_cache(
        ...
        (
            self._store_pipeline.staging_bytes,
            self._store_pipeline.pending_staging_limit_bytes,
        ) = _derive_staging_limits(kv_cache.device)
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        pending_loads = self._load_pipeline.pending
    ...
    def _ensure_load_request_queue(
        ...
        """
        del sample_tensor
        return self._load_pipeline.ensure_queue()
    ...
    def _enqueue_load_request(
        ...
        """
        del load_queue
        self._load_pipeline.enqueue(request)
    ...
    def _ensure_load_request_dispatcher(self, sample_tensor: torch.Tensor) -> None:
        ...
        """
        self._load_pipeline.ensure_dispatcher(
    ...
    def _ensure_load_staging_pool(
        ...
        """
        pool = self._load_pipeline.staging_pool
        if pool is not None:
            return pool
        buffer_bytes = max(
            self._store_pipeline.staging_bytes or DEFAULT_STORE_STAGING_BYTES,
        ...
        self._load_pipeline.set_staging_pool(pool)
        return pool
    ...
    def _submit_load_batch(
        ...
        if self._load_pipeline.staging_registered:
    ...
    def wait_for_save(self) -> None:
        ...
        if commit_keys and reqs_to_store:
            pending_finished = self._store_pipeline.pending_finished_saves
    ...
    def get_block_ids_with_load_errors(self) -> set[int]:
        ...
        """
        invalid_blocks = self._load_pipeline.invalid_block_ids
    ...
    def _collect_finished_loads(self) -> set[str]:
        ...
        for req_id, load in list(pending_loads.items()):
            ...
            try:
                ...
                self._load_pipeline.invalid_block_ids.update(load.block_ids)
                collected_futures.add(future_id)
            finally:
                load.release()
    ...
    def shutdown(self) -> None:
        ...
        for req_id in list(self._store_pipeline.pending_finished_saves):
    ...
    def _submit_load_coroutine(self, coro: Any) -> Any:
        ...
        """
        return self._load_pipeline.submit(coro)
    ...
    def _submit_store_coroutine(self, coro: Any) -> Any:
        ...
        """
        return self._store_pipeline.submit(coro)
    ...
    def _init_server_transfer(self) -> None:
        ...
        self._submit_store_coroutine(
            self._store_pipeline.client.init_transfer()
    ...
    def _load_ipc_clients(self) -> list[Any]:
        ...
        """
        return list(self._load_pipeline.clients())
    ...
    def _load_ipc_client_for_buffer(self, buffer_index: int | None = None) -> Any:
        ...
        """
        return self._load_pipeline.client(buffer_index)
    ...
    def _register_load_staging_buffers(self) -> None:
        ...
        try:
            ...
            self._load_pipeline.staging_registered = False
            return
        self._load_pipeline.staging_registered = True
        logger.info(
    ...
    def _reap_save_futures(self, block: bool) -> None:
        ...
        pending_bytes = self._store_pipeline.pending_staging_bytes
        for record in self._store_pipeline.save_futures:
        ...
        self._store_pipeline.save_futures = remaining
        self._store_pipeline.pending_staging_bytes = pending_bytes
    ...
    def _track_save_future(
        ...
        Async/thread-safety:
            ...
            ``_reap_save_futures``.
        """
        self._store_pipeline.pending_staging_bytes += staging_bytes
        self._store_pipeline.save_futures.append(
    ...
    def _wait_for_save_staging_capacity(self, nbytes: int) -> None:
        ...
        while (
            self._store_pipeline.pending_staging_bytes + nbytes > limit
            and self._store_pipeline.save_futures
        ):
            record = self._store_pipeline.save_futures.pop(0)
            try:
                record.future.result(timeout=120.0)
            finally:
                self._store_pipeline.pending_staging_bytes = max(
                    0,
                    self._store_pipeline.pending_staging_bytes - record.staging_bytes,
                )
                record.release()
            self._reap_save_futures(block=False)
    ...
    def _wait_for_store_staging_release(self, nbytes: int) -> None:
        ...
        """
        pool = self._store_pipeline.staging_pool
        self._wait_for_save_staging_capacity(nbytes)
        while (
            pool is not None
            and pool.available == 0
            and self._store_pipeline.save_futures
        ):
            record = self._store_pipeline.save_futures.pop(0)
            try:
    ...
    def _acquire_staging(
        ...
        if pool is None:
            ...
            self._store_pipeline.staging_pool = pool
        return pool.acquire(
            nbytes,
    ...
    async def _commit_stored_keys(
        ...
        await self._store_pipeline.client.commit_chunks(
            keys_to_commit,
    ...
    async def _transfer_store_cuda(self, **kwargs: Any) -> list[str]:
        ...
        """
        return await self._store_pipeline.client.transfer_store_cuda(**kwargs)
