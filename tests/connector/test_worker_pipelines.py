...
def test_store_pipeline_defers_and_dispatches_up_to_pool_depth() -> None:
...
def test_store_pipeline_streams_request_larger_than_pool_depth() -> None:
    ...
    class Client:
        async def commit_chunks(self, keys: list[str], **kwargs: int) -> None:
            assert keys == ["large"]
            assert kwargs == {"tp_rank": 0, "tp_size": 1}
    ...
    pipeline._client = Client()  # type: ignore[assignment]  # noqa: SLF001
    ...
    pipeline._submit_save = StorePipeline._submit_save.__get__(pipeline)  # noqa: SLF001
    pipeline._kv_caches = {}  # noqa: SLF001
    pipeline.queue_finished({"large": _store_spec("large", [0, 1, 2, 3, 4])}, {"large"})
    pipeline._kv_caches = {"layer": torch.empty(1)}  # noqa: SLF001
