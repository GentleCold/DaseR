...
class _RecordingIPCClient:
    """Captures lookup calls made by the scheduler connector."""
    ...
    def lookup(self, prefix: list[int], model_id: str) -> list[dict[str, Any]]:
        """Record lookup arguments and return configured matches."""
...
class TestSkipSaveFlag:
    def test_skip_save_flag_suppresses_pending_store(self) -> None:
        """``daser_skip_save=True`` must not record pending store intent."""
