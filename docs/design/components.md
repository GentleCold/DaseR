...
| `SchedulerConnectorMixin` | vLLM scheduler | `daser/connector/scheduler.py`；适配 vLLM scheduler hooks，不拥有请求 lifecycle 状态 |
...
| `WorkerConnectorMixin` | vLLM worker | `daser/connector/worker.py`；适配 vLLM worker hooks 和启动期 kernel warmup，不拥有 pipeline 状态 |
...
| worker memory module | vLLM worker | 负责 load/store 共用的 bounded CUDA staging pool、indexed lease 和 capacity derivation |
