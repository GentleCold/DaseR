...
class TestGetNumNewMatchedTokensBug:
    ...
    def test_preallocated_full_prompt_store_skips_alloc_ipc(self):
        """A pre-allocated miss stores after block IDs arrive without re-alloc."""
        BLOCK_TOKENS = 16
        ...
        class MockIPCClientSync:
            ...
            def alloc_chunk(self, chunk_key, token_count, model_id):
                self.alloc_calls.append((chunk_key, token_count, model_id))
                raise AssertionError("preallocated store should not call alloc_chunk")

        mock_ipc = MockIPCClientSync()
        tokens = list(range(260))
        expected_key = hash_tokens(tokens[:256])
        ...
        assert store_spec.num_slots == 16
        assert store_spec.chunk_key == expected_key
        assert store_spec.start_slot == 39
        assert store_spec.file_offset == 39 * 2359296
        assert store_spec.block_ids == list(range(16))
        assert mock_ipc.alloc_calls == []
