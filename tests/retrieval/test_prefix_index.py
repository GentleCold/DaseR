...
# First Party
from daser.retrieval.prefix import (
    PrefixHashIndex,
    _hash_tokens,
    rolling_prefix_keys,
)
...
def make_meta(
    ...
    key = chunk_key or _hash_tokens(tokens)
    return ChunkMeta(
        chunk_key=key,
        start_slot=start,
        num_slots=num,
...
def test_insert_and_exact_lookup():
    ...
    tokens = [1, 2, 3, 4]
    key = rolling_prefix_keys(tokens, block_tokens=4)[0]
...
def test_lookup_longer_query_finds_prefix():
    idx = PrefixHashIndex(block_tokens=4)
    tokens4 = [1, 2, 3, 4]
    key = rolling_prefix_keys(tokens4, block_tokens=4)[0]
...
def test_rolling_prefix_keys_chain_previous_key_with_block_tokens():
    tokens = [1, 2, 3, 4, 5, 6, 7, 8]

    keys = rolling_prefix_keys(tokens, block_tokens=4)
    first_key = rolling_prefix_keys(tokens[:4], block_tokens=4)[0]
    second_key_from_suffix_only = rolling_prefix_keys(tokens[4:8], block_tokens=4)[0]
