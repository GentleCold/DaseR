...
writes a reusable vLLM connector config to
`<store-dir>/daser.connector.json`.
...
Then start vLLM with the generated connector config:
...
| `--store-dir` | (required) | Directory for `daser.store`, `daser.index`, and `daser.connector.json` |
...
Use the JSON file generated by `python -m daser.server`:
